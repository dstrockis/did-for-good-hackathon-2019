...
University.provision().then((identifier: Identifier) => {
  ...
  .then(publicKey => {
    issuerSession = new UserAgentSession(universityIdentifier, 'hard-coded-value-hard-coded-value-ES256K-sig', University.resolver);
  })
})
.catch((error: Error) => {
  console.error(error);
...
router.post('/res', async (context, next) => {
  ...
  if (requestBody) {
    ...
    try {
      selfIssuedToken = await issuerSession.verify(JSON.stringify(requestBody));
    ...
    // Form a student Id
    let studentClaim = formVerifiedStudent(selfIssuedToken.did);
    const credential = await universityIdentifier.sign(studentClaim, 'hard-coded-value-hard-coded-value-ES256K-sig');

    // Create the verified credential and return
    context.body = credential;
    context.status = 200;
  } else {
